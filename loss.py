...
def confidence_rgb_loss(y_true, y_pred):
    ...
    while tf.constant(True, dtype=tf.dtypes.bool):
        ...
        confidence_loss += tf.reduce_sum(-K.log(1.0 + K.epsilon() - K.abs(confidence_true - confidence_pred))) / batch_size_f
...
def yuv_loss(y_true, y_pred):
    ...
    return -K.log(1.0 + K.epsilon() - K.abs(yuv_true - yuv_pred))
